...
exports.update = async (req, res) => {
    try {
        ...
        const template_ = await Templates.findById(tid);

        if (!template_) return res.status(401).json({message: 'Template does not exist'});
        if (req.session['user_id'].toString() !== template_.userId.toString()) return res.status(401).json({message: "Sorry, you don't have the permission to upd this data."});

        const template = await Templates.findByIdAndUpdate(tid, {$set: dt}, {new: true});

        return res.status(200).json({template: template, message: 'Template has been updated'});
